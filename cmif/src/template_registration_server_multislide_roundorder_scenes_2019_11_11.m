...
%
% Intructions: 
%   Use cmif.preprocess.registration_matlab() function to edit and run this template
%
...
for i = 1:length(ls_fname_ref)
       ...
       if sum(contains(d,ls_fname_ref(i))) == 1
            ...
            %get scene name
            fname_scene = cell2mat(ls_fname_ref(i));
            s = strfind(fname_scene, '-Scene-');
            fname_scene = sprintf('%s',fname_scene(s:(u(end-1)-1)));
            ...
            for j=1:length(ls_order)
                ...
                for k = 1:length(fnameall)
                    TF = contains(fnameall(k).name,fname_slide) + contains(fnameall(k).name,fname_scene);
                        if TF == 2
        ...
        %Round 1 write files
            subdirname = sprintf('%s%s%s/',s_subdirname,fname_slide,fname_scene); %write images to folder with slide name

            for l=1:N_colors %for each of R1 files
                long=char(slide(l)); %save name
                u = strfind(long,'_');
                s = strfind(long, '-Scene-');
                first = sprintf('%s',long(1:u(3)-1));
                last = sprintf('%s',long(s:end));
                %write R1 files, don't need to register, just rename
                imwrite(imread(slide{l}),sprintf('%sRegistered-%s%s',subdirname,first,last),'tif', 'Compression','none');
            end
        ...
        for ii= 2: length(DAPI_slide)%for second round DAPI and above (those that need to be registered)
            ...
            if sum(round) == 0
                ...
                for jj=1:length(fname_R)
                    ...
                    imwrite(registered_marker,sprintf('%sRegistered-%s%s',subdirname,first,last),'tif','Compression','none');%write image
